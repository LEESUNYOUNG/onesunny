...
-->
	<head>
		...
		<style>
			...
			#results3 .item:nth-child(1){background:#fa8181;}
			#results3 .item:nth-child(2){background:#ffbc5a;}
			#results3 .item:nth-child(3){background:#fff9c8;}
		</style>
	</head>
	<body>
		...
		어눌,복부에서 불분명한 불편함, 두통,화상을,혈뇨가 나온다,잠깐 동안 의식을,피로와 쇠약,마음이 안정되지,오심(가슴,
...
//------------ define array for Printing (disease or symptom) and Counting (diseaseIndex) ----------------	
	...
	//------------ selected items embeded array print start ----------------
			...
			//----------- %end% 중복성 체크 후 data, countData Insert 완료 --------------	
				...
				console.log('this is countData array: ' + countData);
				console.log('countData array size'+countData.length);
				var t=0;
				for(t=0;t<countData.length;t++)
				{
					console.log('index'+t+'  DiseaseId : '+countData[t]['DiseaseId']+'  count : '+countData[t]['count']);
				}
			...
			//----------- countData개수에 따라서 관련 병 가져오기 -------------
				...
				if($("#content").val())//1,Symptom 상태면 병명찾으러 다녀오기 *해야할 부분*
				{
					setupList(countData);
				}
			}
			...
			function setupList(dataArr){
				...
				$.ajax({
					...
					success: function (response) {
						...
						console.log(myObj2);
						console.log(myObj2.length);

						var str ='';

						for(var i=0;i<myObj2.length;i++){
							str +='<div class="item">'+myObj2[i]['Name']+','+myObj2[i]['DiseaseId']+','+myObj2[i]['Count']+'</div>';
						}
